...
bool ChatHandler::HandleServerInfoCommand(char* /*args*/)
{
    ...
    char const* full;
    if(m_session)
        full = _FULLVERSION(REVISION_DATETIME,REVISION_NR,"|cffffffff|Hurl:" REVISION_ID "|h" REVISION_ID "|h|r");
    else
        full = _FULLVERSION(REVISION_DATETIME,REVISION_NR,REVISION_ID);
    SendSysMessage(full);
